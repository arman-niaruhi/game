...
sphere::sphere(float radius) :
    _radius(radius)
{
    float phi = (2.0f * M_PI) / 20;                                            //the size of phi that we've got in each step and section
    float theta = M_PI / 20;                                                   //the size of theta that we've got in each step and section

    int totaL_number_of_row_sections = 20 + 1;

    for (int v = 0; v <= 20; v++) {                                              // WE need two for loops to go over of all the vertecies that we want to have
        ...
        for (int h = 0; h <= 20; h++) {
            ...
            texcoords.push_back(glm::vec2((1.0f * h) / 20, (1.0f * v) / 20));                           //save all Coordinates in a vector to reuse it for texture mapping
